...
const Profile = ({ currentUser }) => {
        ...
        try {
          ...
          const normalizedFollowers = followersData.map(follower => {
            if (typeof follower === 'string') {
              ...
              return { follower: follower.follower || follower.username || 'Unknown' };
            }
        ...
        if (isOwnProfile) {
          try {
            ...
            if (Array.isArray(requestsData)) {
              ...
              console.error('Follow requests data is not an array:', requestsData);
              setFollowRequests([]);
            }
          } catch (requestsError) {
      ...
      if (isFollowing) {
        ...
        try {
          ...
          if (error.response?.data?.detail?.includes("Already following")) {
            setIsFollowing(true);
          } else if (error.response?.data?.detail?.includes("Follow request already pending")) {
            setFollowRequested(true);
          } else {
            throw error;
          }
  ...
  if (!profile) {
    return <div className="p-6 text-center text-white">Profile not found</div>;
  }
  ...
  const getPhotoUrl = (user) => {
    ...
    const possibleKeys = ['photo', 'userphoto', 'avatar', 'profile_image', 'profileImage', 'image', 'user_photo'];
  
    for (const key of possibleKeys) {
      if (user[key] && typeof user[key] === 'string' && user[key].length > 100) {
        photoData = user[key];
        break;
      }
    }
    ...
    const isLikelyHex = /^[0-9a-fA-F]+$/.test(photoData.replace(/\s/g, '')) && photoData.length % 2 === 0;
    ...
    <div className="p-6">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <div className="h-16 w-16 rounded-full overflow-hidden">
              {getPhotoUrl(profile) ? (
                <img src={getPhotoUrl(profile)} alt="User" className="rounded-full w-24 h-24 object-cover" />
              ) : (
          ...
          {!isOwnProfile && currentUser && (
            ...
            >
              {isFollowing ? 'Unfollow' : followRequested ? 'Request Sent' : 'Follow'}
            </button>
          )}
        </div>
      ...
      <div className="flex border-b border-gray-700 mb-6">
        ...
        <button 
          ...
          className={`px-4 py-2 ${activeTab === 'followers' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-400'}`}
        >
          Followers
        </button>
        <button 
          onClick={() => setActiveTab('following')}
          className={`px-4 py-2 ${activeTab === 'following' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-400'}`}
        >
          Following
        </button>
        {isOwnProfile && (
            ...
            <div>
              {posts.map(post => (
                <div key={post.id} className="bg-gray-800 p-4 rounded-lg mb-4">
                    ...
                    <div className="h-10 w-10 rounded-full overflow-hidden">
                      {getPhotoUrl(post) ? (
                        ...
                        <div className="h-10 w-10 rounded-full bg-blue-500 flex items-center justify-center text-white">
                          {(post.username || username) && (post.username || username).charAt(0).toUpperCase()}
                        </div>
                      )}
                    </div>
                    <div className="ml-3">
                      <p className="font-semibold text-white">{post.username || username}</p>
                  ...
                  <Link to={`/tweet/${post.id}`} className="text-blue-400 hover:underline text-sm mt-2 inline-block">
                    View Post
                  </Link>
                </div>
              ))}
          ...
          {followers.length === 0 ? (
            ...
            <div>
              {followers.map((follower, index) => {
                const followerUsername = typeof follower === 'string' 
                  ? follower 
                    ...
                    <div className="flex items-center">
                      <div className="h-10 w-10 rounded-full overflow-hidden">
                        {getPhotoUrl(follower) ? (
                          ...
                          <div className="h-10 w-10 rounded-full bg-blue-500 flex items-center justify-center text-white">
                            {followerUsername && followerUsername.charAt(0).toUpperCase()}
                          </div>
                        )}
                      </div>
                      <div className="ml-3">
          ...
          {following.length === 0 ? (
            ...
            <div>
              {following.map((follow, index) => {
                const followUsername = typeof follow === 'string' ? follow : 
                  follow && follow.following ? follow.following : 
                  follow && follow.username ? follow.username : 
                  `Unknown-${index}`;
                    ...
                    <div className="flex items-center">
                      <div className="h-10 w-10 rounded-full overflow-hidden">
                        {getPhotoUrl(follow) ? (
                          ...
                          <div className="h-10 w-10 rounded-full bg-blue-500 flex items-center justify-center text-white">
                            {followUsername && followUsername.charAt(0).toUpperCase()}
                          </div>
                        )}
                      </div>
                      <div className="ml-3">
