      ...
      <div className="flex items-start">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            <div>
              <Link to={`/profile/${tweet.username}`} className="font-medium text-white hover:underline">
                {tweet.author}
              </Link>
              <span className="text-gray-400 text-sm ml-2">@{tweet.username}</span>
            </div>
          ...
          <div className="mt-3 flex space-x-6 text-gray-400">
            <button onClick={handleLike} className={`flex items-center space-x-1 ${isLiked ? 'text-red-500' : 'hover:text-red-400'}`}>
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <Link to={`/tweet/${tweet.tweetid}`} className="flex items-center space-x-1 hover:text-blue-400">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
